...
local function filter_non_project_file_diagnostics(_, result, ctx, config)
    if result and result.diagnostics then
        result.diagnostics = vim.tbl_filter(function(d)
            -- Only keep diagnostics that aren't the "non-project" warning
            return not string.match(d.message, "is a non%-project file")
...
return {
    ...
    handlers = {
        ...
        -- https://github.com/eclipse/eclipse.jdt.ls/issues/376
                ["textDocument/publishDiagnostics"] = filter_non_project_file_diagnostics,
