...
return {
						...
						i = { -- copy highlight values
							["<CR>"] = function(promptBufnr)
								...
								local out = vim.iter(hlValue):fold({}, function(acc, key, val)
									...
									if key == "fg" then
										acc.fg = ("#%06x"):format(val)
									end
									if key == "bg" then
										acc.bg = ("#%06x"):format(val)
									end
									return acc
								end)
	...
	keys = {
		...
		},
		{
			"<leader>qh",
			function()
				vim.cmd.Telescope("quickfixhistroy")
			end,
		...
		},
		{
			"gP",
			function()
				local projects = vim.iter(vim.fs.dir(vim.g.projects_dir)):fold({}, function(acc, item, type)
		...
		},
		{
			...
			function()
				vim.cmd("noautocmd Telescope colorscheme")
			end,
