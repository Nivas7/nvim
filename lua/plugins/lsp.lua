...
vim.api.nvim_create_autocmd("LspAttach", {
  ...
  callback = function(event)
    ...
    if client and client.server_capabilities.documentHighlightProvider then
      local highlight_augroup = vim.api.nvim_create_augroup("_lsp-highlight", { clear = false })
    ...
    vim.api.nvim_create_autocmd("LspDetach", {
      group = vim.api.nvim_create_augroup("_lsp-detach", { clear = true }),
      callback = function(event2)
        vim.lsp.buf.clear_references()
        vim.api.nvim_clear_autocmds({ group = "_lsp-highlight", buffer = event2.buf })
      end,
    })
...
local signs = { Error = icons.diagnostics.Error, Warn = icons.diagnostics.Warn, Hint = icons.diagnostics.Hint, Info = icons.diagnostics.Info }
